...
class OAuth extends Component<FixMeLater, { disabled: boolean }> {
	...
	startAuth = () => {
		...
		socket.on(provider, (res: FixMeLater) => {
			try {
				if (route === '') {
					if (res.user === null) throw res.data || 'Something went wrong!';
		...
		if (!this.state.disabled) {
			...
			Axios.get(`${API_URL}/api/oauth${route}/${provider}?${provider}SocketId=${socket.id}`)
				.then((data) => {
...
const mapDispatchToProps = (dispatch: Dispatch<Action>) => ({
	addSocialAccount: (socialAccount: FixMeLater) => dispatch({ type: ADD_SOCIAL_ACCOUNT, socialAccount }),
});
