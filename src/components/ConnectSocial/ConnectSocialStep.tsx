					...
					<Grid className="social-connect-row">
						<div className="social-wrapper">
							<Grid item className="social-connect-heading">
								<h2 className="social-connect-text">You are almost there!</h2>
								<p className="social-connect-paragraph">To begin using Buzzroom, just attach a social profile</p>
								<div className="connect-social-button-wrapper">
									{providers.map(({ name, classNames, style, text, icon }) => (
										<OAuth socket={socket} provider={name} key={name} route="/link" history={props.history}>
											<Button className={`social-${classNames}`} style={style}>
												<FontAwesomeIcon icon={icon} className="social-icons"></FontAwesomeIcon>
												{text}
											</Button>
										</OAuth>
									))}
